...
class List extends Component {
  ...
  showList(data) {
    return this.props.children || <Dashboard courses={data} notFromHome={true} route="group"/>
  }
  ...
  render() {
    const {columns, children, className,course,items} = this.props;
    return (
      <div className={`list columns-${columns} ${className}`}>
        { course?this.showList(items):this.showListItems() }
        { children }
      </div>
    );
  }
